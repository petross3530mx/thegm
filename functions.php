      ...
      foreach ($imagesInContent as $image) {
        ...
        if (strpos($imageClass, 'not_social') === false) {
         ...
         // $pinterestUrl .= rawurlencode($utm_pinterest);
          ...
          $pinBtn->appendChild($doc->createTextNode('PIN IT'));
...
function getPrimeCatPost ($count, $category) {
...
/*
*	Get single random post of sidebar.
*
*
*/
function getPostdataSidebar ($count, $category) {
  global $random_posts_id;

  // get the prime category id
  $categories = get_the_category($post_id);
  $cat_isnt_one = count((array)$categories)>1;

  if( $cat_isnt_one and yoast_get_primary_term_id() ): // primary category is defined
    $category_id  = yoast_get_primary_term_id();
...
/*
*	Generate prime category name
*
*/
function showPrimeCatName(){
    ...
    <div class="rf-item rf-feature-<?php echo $i; ?> <?php echo addGridItemClasses($i); ?>">
      ...
      <div class="rf-article-meta">
      	<div class="rf-meta-inner">
	        <span class="h4-foot-replacement"><?php echo get_the_title($post_id); ?></span>
...
function renderSinglePost ($posts) {
  ...
  foreach ($posts as $post) { setup_postdata($post); 
    ...
    <div class="article_holder cb-footer cb-feature-<?php echo $i; ?> <?php echo addGridItemClasses($i); ?>">
      ...
      <div class="cb-article-meta">
        <span class="h4-replacement"><a href="<?php the_permalink($post_id) ?>"><?php echo get_the_title($post_id); ?></a></h4>
        <?php echo cb_byline(true, $post_id); // print all category for current post ?>
      </div>
    </div>
  <?php $i++;
...
if (!function_exists('post_cat_list')) {
  function post_cat_list ($post_cats, $sep = ',') {
    $cat_iter = 1;
    $output = '';
...
function generateFirstAdBlock () {
  return '<div class="cut-off-ad" style="margin: 30px 0px; text-align: center;">
            ...
            <div data-aaad="true" data-aa-adunit="/22181265/Glam_728v_1"></div>
            <div data-aaad="true" data-aa-adunit="/22181265/Glam_MOB_300v_1"></div>
  </div>';
}
...
function wf_primary_id($post_id){
    ...
    if($cat_isnt_one and yoast_get_primary_term_id()):
...
function wf_primary_category($post_id){
    ...
    if($cat_isnt_one and yoast_get_primary_term_id()):
        $prim_cat_id = yoast_get_primary_term_id();
...
function glam_breadcrumbs($post_id){
  ...
  if( $cat_isnt_one and yoast_get_primary_term_id() ): // primary category is defined
    ...
    $prime_id    = yoast_get_primary_term_id();
    $child_term  = get_term($prime_id, 'category');
    $parent_term = get_term($child_term->parent, 'category');

    $curr_cat = yoast_get_primary_term_id();

    $child_catname  = esc_html($child_term->name);
    $child_catlink  = get_category_link($prime_id);

  if(category_has_parent($curr_cat)):
        $parent_catname = esc_html($parent_term->name);
        $parent_catlink = esc_url(get_category_link($parent_term->term_id));
  endif;

  elseif(category_has_parent($curr_cat)):
    $prime_id    = $categories[0]->term_id;
  ...
  if(category_has_parent($curr_cat)):
    $result .= '<li itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem"> 
                <a itemprop="item" href="'.$parent_catlink.'"><span itemprop="name">'.$parent_catname.'</span></a>
                <meta itemprop="position" content="'.$cat_lvl.'">
                </li> &raquo; ';
    $cat_lvl = 3;
  endif;
...
/*
 * Defines category breadcrums Scheme data
 * has some conditions if category has parent
 */
function category_breadcrumbs(){
  $category       = get_queried_object();
  $child_catname  = esc_html($category->name);
  $child_catlink  = get_category_link($category->term_id);
  $cat_lvl = 2;
  ...
  $result .= '<li itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem">
                  <span itemprop="name">'.$child_catname.'</span>
                  <meta itemprop="position" content="'.$cat_lvl.'">
              </li>';
  $result .= '</ol>';
  echo $result;
...
/*
 * Register shortcode [faq]
 *
 * Decription: "Creating a list of Frequently Asked Questions"
 *
 */
function faq_func(){
    if(get_field('faq_block')):
        $a = get_field('faq_block');
        $arr = preg_split('/(<h3>.*?<\/h3>)/', $a, -1,  PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
        // $title = array_shift($arr);
        $items = '';
        
        for ($i = 0; $i < count($arr); $i+=2) {
            // $ques = $arr[$i];
            $ques = str_replace("<h3>","<h3 itemprop=\"name\">", $arr[$i]);
            $answ = $arr[$i+1];
            $items .=
...
/*
 * Register shortcode [bra]
 *
 * Decription: "Injecting bra Size Calculator"
 *
 */
function bra_func(){
    ob_start();
    get_template_part('components/bra/bra', 'calculator' );
    return ob_get_clean();
}
add_shortcode('bra', 'bra_func');


/*
 * Changing the view of pagination
 *
 */
