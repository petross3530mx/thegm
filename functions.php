...
function glaminati_scripts() {
  ...
  wp_enqueue_script( 'my-js', get_template_directory_uri() . '/js/my.js', array('jquery'), '20151217', true );
  ...
  //DB-DISABLE: wp_enqueue_script( 'sticky', get_template_directory_uri() . '/js/jquery.sticky.js', array('jquery'), '20151215', true );

  //DB-DISABLE: wp_enqueue_script( 'glaminati-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array('jquery'), '20151215', true );
}
 ...
 * Help function.
 */
require get_template_directory() . '/theme-inc/disable_comments.php';

/**
 * Implement Helpers feature.
 */
      ...
      foreach ($imagesInContent as $image) {
        ...
        if (strpos($imageClass, 'not_social') === false) {
         ...
         // $pinterestUrl .= rawurlencode($utm_pinterest);
          $pinterestUrl .= '&media=' . rawurlencode($imageSrc);
          ...
          $pinBtn->appendChild($doc->createTextNode('Pinterst'));
...
function getSingleRandomPost ($count, $category) {
  ...
  $posts = get_posts($args);
  // print_r(get_field('related_post_list', 'option'));
  return $posts;
}
...
function faq_func(){
    if(get_field('faq_block')):
        ...
        $str =
            '<div class="faq tac" itemscope="" itemtype="https://schema.org/FAQPage">
              '.$items.'
            </div>';
