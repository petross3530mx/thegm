...
if(get_post_meta(get_the_ID(), '_yoast_wpseo_primary_category',true)):
	$category = get_post_meta(get_the_ID(), '_yoast_wpseo_primary_category',true);
else:
...
<article itemscope itemtype="http://schema.org/Article" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
				...
				<?php //if($_GET['test']): ?>
				  <ol itemscope itemtype="https://schema.org/BreadcrumbList" class="nav_breadcrumbs">
		            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"> 
		      	    ...
		      	    <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
		                      <a itemprop="item" href="<?=wf_primary_catlink(get_the_ID());?>"><span itemprop="name"><?=wf_primary_category(get_the_ID());?></span></a>
	...
	<div class="entry-content-article-container cb-cat-container" itemprop="articleBody">
		...
		<?php
			...
			//DB: temp fix while we removed the ads
			//!!! something is broken below with CUT-OFF, LazyLoading is not working with CUT-OFF.
			//Likely resulting from poorly formed HTML
			$adsMarker  = "<!--cut_off-->";
			$pieces = explode($adsMarker, $content);
			$counter = count($pieces);


			$ad_id = array('1545338539298-0','1545423757024-0','1545423810799-0','1546600851669-0', '1546600875945-0');
			$s_ad_id = array('1545426903960-0','1545427439313-0','1545428270736-0','1546600824028-0','1546600794923-0');
				...
				<?php if($i==0){ 
					...
					<aside id="secondary" class="widget-area">
							...
							<div class="relatedposts-container">
								<div class="mk_related_caption">More from <?php echo wf_primary_category(get_the_ID()); ?></div>
								<div class="new_related clearfix">
									<?php
									$posts = getPrimeCatPost($count = 5, intval($category));
